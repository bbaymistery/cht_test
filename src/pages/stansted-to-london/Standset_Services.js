...
const Standset_Services = (props) => {
    ...
    const addPointCollectPoints = (index) => {
        ...
        fetch(url, configTransfer)
            ...
            .catch((error) => {
                let message = "Churchill Standset_Services component _addPointCollectPoints()  function catch blog  "
    ...
    useEffect(() => {
        ...
        if (populationDestinations["stansted-london"].quotations.length > 0) {
            // gtLondon = populationDestinations["stansted-london"]

            setFirstAdresQuotations(populationDestinations["stansted-london"].quotations)
            //POINTS for hgttolondo
            setpickPointFirstAdress(populationDestinations["stansted-london"].selectedPickupPoints[0])
            setdropPointFirstAdress(populationDestinations["stansted-london"].selectedDropoffPoints[0])

            setgtsecondAdressQuotations(populationDestinations["london-stansted"].quotations)
        }
    }, [])
        ...
        <div className="service-rate-table-wrapper clearfix">
            <div className="service-rate-table-inner-wrapper clearfix">
                ...
                <div className="service-rate-wrapper">
                    <div className="service-rate-header"><p><strong>Stansted to London</strong></p></div>
                    <div className="service-rate-inner">
                        {secondAdressQuotations?.length > 0 ? secondAdressQuotations?.map((quotation, index) => {
                            return <div key={index} className="service-rate-section">
                                <p><strong><span>£{quotation.price}</span> / All Inclusive</strong> * meet and greet included</p>
                            </div>
                        }) : <React.Fragment></React.Fragment>}
                    </div>
                </div>
                ...
                <div className="service-rate-wrapper">
                    <div className="service-rate-header"><p><strong>London to Stansted</strong></p></div>
                    <div className="service-rate-inner">
                        {firstAdresQuotations?.length > 0 ? firstAdresQuotations?.map((quotation, index) => {
                            return <div key={index} className="service-rate-section">
...
export default Standset_Services